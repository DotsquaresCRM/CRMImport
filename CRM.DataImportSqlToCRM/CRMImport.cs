...
namespace DSTools.MSSqlToCRMImport
{
    public partial class CRMImport : PluginControlBase, IGitHubPlugin, ICodePlexPlugin, IPayPalPlugin, IHelpPlugin, IStatusBarMessenger, IShortcutReceiver, IAboutPlugin
    {
        ...
        private List<Entity> errors = new List<Entity>();
        ...
        {
            int size = Convert.ToInt32(txtimport.Text.Trim());
            if (size > 1000 || size == 0)
            {
                MessageBox.Show("Max. 1000 Batch Size allow.Please Change it.");
            }
            else
            {
        ...
        public void LoadTableRecord()
        {
            ...
            long recordCount = 0;
            int PageNumber = 1;
            int PageSize = Convert.ToInt32(txtimport.Text);
            cnn = new SqlConnection(Connection.ConnectionString);
            cnn.Open();
            using (SqlCommand cmd = new SqlCommand("Select count(*) from " + cmbTables.SelectedItem.ToString(), cnn))
            {
                using (SqlDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        recordCount = Convert.ToInt64(dr[0].ToString());
                    }
                }
            }
            bool isExecute = true;
            ...
            if (recordCount > 0)
            {
                ...
                do
                {
                    ...
                    {
                        query = query + item.ToString().Split(':')[0].Trim() + ",";
                    }
                    query = query.Substring(0, query.Length - 1);
                    List<Entity> dbEntity = new List<Entity>();
                    using (SqlCommand cmd = new SqlCommand("Select " + query + " from " + cmbTables.SelectedItem.ToString() + " order by " + lstSortedTablesColumn.Items[0].ToString().Split(':')[0].Trim() + " OFFSET  " + PageSize + " * (" + PageNumber + " - 1) ROWS FETCH NEXT " + PageSize + " ROWS ONLY OPTION (RECOMPILE)", cnn))
                    {
                        using (SqlDataReader dr = cmd.ExecuteReader())
                        {
                            if (dr.HasRows == false)
                            ...
                            while (dr.Read())
                            {
                                ...
                                foreach (var item in lstSortedEntityColumn.Items)
                                {
                                    ...
                                    {
                                        case "Boolean":
                                            en[item.ToString().Split(':')[1].Trim()] = Convert.ToBoolean(dr[lstSortedTablesColumn.Items[i].ToString().Split(':')[0].Trim()]);
                                            break;
                                        case "DateTime":
                                            en[item.ToString().Split(':')[1].Trim()] = DateTime.Parse(dr[lstSortedTablesColumn.Items[i].ToString()].ToString().Split(':')[0].Trim());
                                            break;
                                        case "Integer":
                                            en[item.ToString().Split(':')[1].Trim()] = Convert.ToInt32(dr[lstSortedTablesColumn.Items[i].ToString()].ToString().Split(':')[0].Trim());
                                            break;
                                        case "BigInt":
                                            en[item.ToString().Split(':')[1].Trim()] = Convert.ToInt64(dr[lstSortedTablesColumn.Items[i].ToString()].ToString().Split(':')[0].Trim());
                                            break;
                                        case "Money":
                                            en[item.ToString().Split(':')[1].Trim()] = new Money(Convert.ToDecimal(dr[lstSortedTablesColumn.Items[i].ToString()].ToString().Split(':')[0].Trim()));
                                            break;
                                        case "String":
                                            en[item.ToString().Split(':')[1].Trim()] = dr[lstSortedTablesColumn.Items[i].ToString().Split(':')[0].Trim()];
                                            break;
                                    }
                    ...
                    //if (recordCount > ((PageNumber - 1) * PageSize) + dbEntity.Count)
                    //{
                    //    isExecute = false;
                    //}
                } while (isExecute);
            ...
            {
                MessageBox.Show("No Record Found in MS SQL Server.");
            }
        }
        ...
        {
            int size = Convert.ToInt32(txtimport.Text);
            if (size > 1000)
            {
                MessageBox.Show("Max. 1000 Batch Size allow.");
            }
        }
    }
}